import './randomChar.scss';
import thor from '../../resources/Thumbnailthor.jpg';
import image from '../../resources/Decorationmjholnir.png';

const RandomChar = () => {
return (
   <div className="container">
      <div className="randomchar">
         <div className="randomchar__dynamic">
            <img src={thor} alt="" className="randomchar__thumbnail" />
            <div className="randomchar__descr">
               <h3 className="randomchar__subtitle">THOR</h3>
               <p className="randomchar__text">As the Norse God of thunder and lightning, Thor wields one of the greatest weapons ever made, the enchanted hammer Mjolnir. While others have described Thor as an over-muscled, oafish imbecile, he's quite smart and compassionate...</p>
               <a href="#" className="randomchar__homepage button"><span className="randomchar__inner">HOMEPAGE</span></a>
               <a href="#" className="randomchar__wiki">WIKI</a>
            </div>
         </div>
         <div className="randomchar__static">
            <p className="randomchar__today">Random character for today! <br/>Do you want to get to know him better?</p>
            <p className="randomchar__choice">Or choose another one</p>
            <button className="randomchar__generate">TRY IT</button>
            <img src={image} alt="" className="randomchar__image" />
         </div>
      </div>
   </div>
)
}

export default RandomChar;